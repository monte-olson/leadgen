 ...
 */
function ConvertJsonToTable(parsedJson, tableId, tableClassName, linkText,header)  //Added header parameter to allow custom headers
{
    ...
    var tbl = '<table border="1" cellpadding="4" cellspacing="1"' + idMarkup + classMarkup + '>{0}{1}</table>';

    //Patterns for table content
    var th = '<thead>{0}</thead>';
    var tb = '<tbody>{0}</tbody>';
    var tr = '<tr>{0}</tr>';
    var tr1 = '<tr class="alt">{0}</tr>';
    var thRow = '<th>{0}</th>';
    var tdRow = '<td>{0}</td>';
    var thCon = '';
    var tbCon = '';
    ...
    if (parsedJson)
    {
        ...
        else
        {
            ...
            {
                if (typeof (header) != 'object')  {   //use Json key values for table display
                    headers = array_keys(parsedJson[0]);
                    for (i = 0; i < headers.length; i++)
                        thCon += thRow.format(headers[i]);
                }
                else {               //Use the value passed on to create header values
                    headers = array_keys(parsedJson[0]);
                    for (i = 0; i < header.length; i++)
                        thCon += thRow.format(header[i]);
                }
            }
        }
